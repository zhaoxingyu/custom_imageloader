...
import com.sina.weibo.universalimageloader.core.assist.FailReason.FailType;
...
import com.sina.weibo.universalimageloader.core.decode.ImageDecodingInfo;
 ...
 * display it in {@link com.sina.weibo.universalimageloader.core.imageaware.ImageAware} using {@link DisplayBitmapTask}.
 *
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		Bitmap bitmap = null;
		try {
			File imageFile = configuration.diskCache.get(uri, options.getDiskCacheSubDir());
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				...
				if (options.isCacheOnDisk() && tryCacheImageOnDisk()) {
					imageFile = configuration.diskCache.get(uri, options.getDiskCacheSubDir());
					if (imageFile != null) {
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		boolean saved = false;
		File targetFile = configuration.diskCache.get(uri, options.getDiskCacheSubDir());
