...
import com.sina.weibo.universalimageloader.core.DefaultConfigurationFactory;
import com.sina.weibo.universalimageloader.utils.FileUtils;
...
public abstract class BaseDiscCache implements DiskCache {
    ...
    @Override
    public File getDirectory(DiskCacheFolder folder) {
        File file = new File(getDirectory(), folder.getFolderName());
        if (!file.exists()) {
            file.mkdirs();
        }
        return file;
    ...
    public boolean remove(String imageUri) {
        return remove(imageUri, DiskCacheFolder.DEFAULT);
    }

    @Override
    public boolean remove(String imageUri, DiskCacheFolder folder) {
        return getFile(imageUri, folder).delete();
    }
    ...
    @Override
    public void clear() {
        for (DiskCacheFolder folder : DiskCacheFolder.values()) {
    ...
    protected File getFile(String imageUri) {
        return getFile(imageUri, DiskCacheFolder.DEFAULT);
    }

    protected File getFile(String imageUri, DiskCacheFolder folder) {
        return get(imageUri, folder);
    }
